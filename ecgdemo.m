...
% Define the range limit for plotting based on average distance between peaks
rangeLimit = round(averageDistanceBetweenPeaks * 5); % Adjust this value as needed
...
[P_wave, T_wave] = extract_P_T_wave(ecg, positions2, meanDistance, samplingrate, rangeLimit);
analyze_P_T(P_wave, T_wave, meanDistance, samplingrate, rangeLimit);
...
% figure;
% scatter(RR_intervals(1:min(end-1, rangeLimit)), RR_intervals(2:min(end, rangeLimit+1)), 'filled');
% xlabel('RR Interval n (s)');
% ylabel('RR Interval n+1 (s)');
% title('Poincaré Plot of RR Intervals');

figure;
plot(ecg(1:round(rangeLimit)), '-b'); hold on;
stem(valid_peaks(valid_peaks <= round(rangeLimit)), ecg(valid_peaks(valid_peaks <= round(rangeLimit))), 'or', 'LineWidth', 1.5);
